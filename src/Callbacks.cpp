...
int Callbacks::adsCallback(void *ptr, int argc, char **argv, char **colNames){
    ...
    currentAd->setSellerUsername(argv[3]);
    currentAd->setSellerId(atoi(argv[4]));
    currentAd->setCity(argv[5]);
    currentAd->setState(argv[6]);
    currentAd->setPrice(atof(argv[7]));
    currentAd->setDate(argv[8]);
    currentAd->setAmount(atoi(argv[9]));
    currentAd->setDescription(argv[10]);
